...
function UpdateModal(props: IBlogModalProps) {
  const { open, setOpen, blog, resetSelected } = props;
  ...
  const {
    ...
    defaultValues: useMemo(() => {
      const { title, summary, content, tag, topic } = blog;
      return {
        title,
        summary,
        content,
          ...
          <Grid item xs={12} md={7}>
            <Grid container justifyContent={"space-between"}>
              <Grid item xs={5.8} mb={3}>
                <InputLabel sx={{ mb: 1 }}>Chủ Đề</InputLabel>
                <Controller
                  defaultValue={0}
                  name="topicId"
                  control={control}
                  render={({ field }) => (
                    <Select fullWidth {...field} variant="outlined">
